...
6. **Hand off to `loom:writing-workflows`** by invoking:
   ...
   ```
   Set `finalize` to `"merge"` only if the original `/story` request explicitly asked to merge to main (e.g. "merge to main", "push to main", "no PR"); otherwise use `"pr"` (the default). That skill generates a bespoke baked-DAG workflow script and launches it. The generated workflow dispatches one story-executor, runs review and validation, and finalizes the branch. On any non-ok result, follow the **HALT PROTOCOL** below.

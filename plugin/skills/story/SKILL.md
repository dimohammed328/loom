...
6. **Hand off to `loom:writing-workflows`** by invoking:
   ...
   ```
   `finalize` is derived mechanically from the original `/story` request text — it is NEVER a question for the user. Use `"merge"` only when the request explicitly asked to merge (e.g. "merge to main", "push to main", "no PR"); in every other case — including when the request says nothing about merging — use `"pr"`. Do NOT ask the user to choose between PR and merge, in prose or via AskUserQuestion. Unsure means `"pr"`. That skill generates a bespoke baked-DAG workflow script and launches it. The generated workflow dispatches one story-executor, runs review and validation, and finalizes the branch. On any non-ok result, follow the **HALT PROTOCOL** below.
